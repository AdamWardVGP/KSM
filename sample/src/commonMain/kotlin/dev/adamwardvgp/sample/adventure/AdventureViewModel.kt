...
class AdventureViewModelImpl(
    private val savedStateHandle: SavedStateHandle
) : ViewModel(), AdventureViewModel {

    private val restoredState: AdventureState? =
        savedStateHandle.get<String>(KEY_STATE)
            ?.let { runCatching { Json.decodeFromString<AdventureState>(it) }.getOrNull() }

    private val stateMachine = getAdventureStateMachine(viewModelScope, restoredState ?: AdventureState.Start)
